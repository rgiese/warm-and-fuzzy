...
public:
    ...
    static bool RetrieveMeasurement(__out float& Celsius, OneWireAddress const& Address, IOneWireGateway const& OneWireGateway)
    {
        // Reset bus and select device by address
        RETURN_IF_FALSE(OneWireGateway.Reset());
        RETURN_IF_FALSE(OneWireGateway.SelectAddress(Address));
    ...
    // All-in-one function
    static bool ReadTemperature(__out float& Celsius, OneWireAddress const& Address, IOneWireGateway const& OneWireGateway)
    {
        // Request measurement
